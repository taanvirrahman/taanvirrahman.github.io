...
export const renderResourceList = (resources) => {
    ...
    <article class="resource-item group flex flex-col md:flex-row gap-6 py-6 border-b transition-all duration-300 transform" data-resource-id="${resource.id}" role="button" tabindex="0" style="border-color: var(--resource-border)">
      ...
      <div class="resource-thumbnail-container shrink-0 w-full md:w-[160px] h-[100px] overflow-hidden rounded-sm" style="background-color: var(--resource-thumb-bg)">
          <img src="${resource.thumbnail}" alt="${resource.title}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
      </div>` : ""}
      <div class="flex-1 flex flex-col justify-center">
          ...
          <div class="resource-snippet text-[14px] leading-relaxed mb-2 transition-all duration-300" style="color: var(--resource-text-snippet)">
            ${resource.snippet || 'Exploring the boundaries of technical innovation and design excellence...'}
          </div>
...
export const showResource = (resource) => {
    ...
    <div class="mb-12 w-full max-w-4xl mx-auto overflow-hidden rounded-sm" style="background-color: var(--resource-thumb-bg)">
      <img src="${resource.thumbnail}" alt="${resource.title}" class="w-full h-auto object-cover max-h-[500px]" />
    </div>` : ""}
`;
    ...
    if (typeof marked !== "undefined") {
        ...
        const contentHTML = `< div class="max-w-3xl mx-auto" > ${marked.parse(resource.content)}</div > `;
